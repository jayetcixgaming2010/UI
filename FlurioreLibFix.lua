  ...
  DropShadowHolder.Position = UDim2.new(0, (HirimiGui.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0, (HirimiGui.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
	...
	MaxRestore.Activated:Connect(function()
		...
		if ImageLabel.Image == "rbxassetid://9886659406" then
			...
			-- ensure visible and enabled when maximizing
			DropShadowHolder.Visible = true
			if HirimiGui and HirimiGui:IsA("ScreenGui") then
				pcall(function() HirimiGui.Enabled = true end)
			end
			...
			ImageLabel.Image = "rbxassetid://9886659406"
			-- restore and ensure visible/enabled
			DropShadowHolder.Visible = true
			if HirimiGui and HirimiGui:IsA("ScreenGui") then
				pcall(function() HirimiGui.Enabled = true end)
			end
	...
	Min.Activated:Connect(function()
		...
		-- hide the main holder and disable the entire ScreenGui so external toggles can re-enable it
		DropShadowHolder.Visible = false
		if HirimiGui and HirimiGui:IsA("ScreenGui") then
			pcall(function() HirimiGui.Enabled = false end)
		end
	end)
	Close.Activated:Connect(function()
		CircleClick(Close, Mouse.X, Mouse.Y)
		-- same as Min: disable the ScreenGui
		DropShadowHolder.Visible = false
		if HirimiGui and HirimiGui:IsA("ScreenGui") then
			pcall(function() HirimiGui.Enabled = false end)
		end
